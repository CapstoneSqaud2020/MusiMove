...
client = pymongo.MongoClient("mongodb+srv://CapstoneCrew:<pw>@cluster0-lvmuw.azure.mongodb.net/test?retryWrites=true&w=majority")
...
def userExists(username):
    ...
    if(acc.count_documents({"usrnm":username})>0):
        return 1
    else:
        return 0 
...
def login(username, password):
    acc = db.account
    user = acc.find({"usrnm":username, "pw":password})
